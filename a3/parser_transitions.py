...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            pair = (word1, word2) if transition == 'LA' else (word2, word1)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        for i in range(len(minibatches)):
            minibatches[i].parse_step(transitions[i])
        unfinished_batches = []
        for minibatch in minibatches:
            if minibatch.buffer or len(minibatch.stack) != 1:
                unfinished_batches.append(minibatch)
        unfinished_parses = unfinished_batches + unfinished_parses[batch_size:]
    ### END YOUR CODE
